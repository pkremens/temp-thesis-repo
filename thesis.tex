...
RAM: 8GB\\
Processor: Intel i7-2620M Quad Core\\
V této kapitole si ukážeme 

Před zahájením testu a okamžitě po jeho skoncíní změříme paměťové otisky serveru (množství, paměti které program používá ke svému běhu). Budou nás zajímat následující hodnoty:
\begin{itemize}
  \item
  \item 
\end{itemize}
Před každým měřením zavoláme pomocí JMX~\footnote{Java Management Extensions - technologie podporující management a monitoring aplikací} operaci forceGC(), abychom se ujistili, ze v pameti zustanou jen nezbytné objekty 
...
Při testování aplikace jsem se potýkal s problémy s transakcemi. Z důvodu délky transakčních dávek a obousměrných vazeb mezi entitami docházelo k uváznutí a zrušení transakcí. Na JBossu se mi podařilo tento problém obejít zakomentováním atributu stored u Produktu, ovšem na serveru GlashFish ani tato změna tento problém 100\% nevyřešila a jedna z dvaceti transakcí byla zrušena. Problém by snad vyřešila specifická úprava nastavení poskytovatele JPA nebo fondu JDBC. Dalším z možných řešení by mohl být přechod z transakcí řízených kontejnerem na transakce řízené beanou.
