...
výkonnostní testování, aplikační server, benchmarking, Java EE 6, JPA, EJB, JSF, JMS, 
\end{ThesisKeyWords}
...
V třívrstvé architektuře mohou klientské aplikace (běžící ve webovém konteineru) a session beany (běžící v EJB konteineru) sdílet stejné prostředí (aplikační server), mohou ovšem také běžet na separátních strojích.
...
\begin{itemize}
  ...
  \item {\bf Message driven beana} - použity pro integraci s externími systémi pomocí JMS
  \item {\bf Entity beana} -  používány pro persistenci dat (označeny k prořezání, viz \ref{pruning})
\end{itemize}

Bezstavová beany, stavové beany a Singleon sborně nazýváme Session beanami.
...
\begin{itemize}
  \item Git
  \item Apache Maven (verze alesoň 3.0)
...
\begin{itemize}
  \item stahneme si zdrojove kódy z repozitáře:
\begin{lstlisting}
...
K tomu, aby mohla naše aplikace mohla pracovat s databází, musíme serveru nastavit datový zdroj. V této ukázce použijeme databázy MySQL.
Nejprve musíme přidat nový modul\footnote{JBoss AS 7 poskytuje system modulu, aby zajistil isolaci pri načítání tříd do paměti~\cite{datasource}}
\begin{lstlisting}
$ cd $JBOSS_HOME
$ mkdir -p modules/com/mysql/main
$ cd modules/com/mysql/main
\end{lstlisting}
Knihovnu s ovladačem k databázi zkopírujem na tuto lokaci a vytvoříme nový soubor module.xml, s následujícím obsahem\footnote[1]{verzi knihovny upravíme tak, aby seděla s tou, kterou se chystáme použít}:
\begin{lstlisting}
...
\end{lstlisting}
Nyní když máme vytvořený modul, musíme na něj udělat odkaz v hlavním konfigarčním souboru serveru:
\begin{lstlisting}
...
\end{lstlisting}
Ve stejném souboru musíme také nastavit datový zdroj:
\begin{lstlisting}
<datasource
...
\end{lstlisting}
\subsection{JMS}
K využití služeb naší MDB musíme nastavit frontu, které bude uchovávat zprávy vyprodukované naší aplikací. Ke konfiguraci JMS fronty využijeme CLI (Command Line Interface), což je jedno z rozhraní na spravování serveru, které JBoss AS nabízí. Nejprve spustíme server:
\begin{lstlisting}
$ cd $JBOSS_HOME/bin
...
Nyní když máme spuštěný server nám již nebrání nic k nasazení aplikace. Použijeme pluginy install a jboss-as nástroje Maven, které vykonají veškerou práci za nás:
\begin{lstlisting}
...
\end{lstlisting}
Máme-li k dispozici nejaký nástroj na generování zátěže, můžeme ke generování použít servlet na adrese .... TODO  d
