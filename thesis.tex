...
\chapter*{Úvod}
Úspěch velkých firem moderní doby žije a skomírá s výkonností aplikací, které jsou pro jejich podnikání nezbytné. Bohužel, mnoho aplikací je používáno, aniž je adekvátně testována jejich škálovatelnost a výkonnost. Proces výkonnostního testování pomáhá identifikovat slabiny aplikace ještě předtím, než je nasazena do produkčního prostředí. Během výkonnostního testování simulujeme práci klientů na testované aplikaci a monitorujeme její chování. Cílem výkonnostního testování není hledat chyby, ale eliminovat místa, která negativně ovlivňují výkonnost aplikace, a zároveň určit počáteční hodnoty klíčových cílů výkonnosti, které budou použity jako základ budoucího regresního testování.

Aplikační servery jsou páteřní komponentou pro chod mnoha společností. Umožňují rychlé a spolehlivé sdílení dat, centralizovanou distribuci software, jednoduchou modernizaci, vysokou míru zabezpečení a zvyšují efektivitu všech firemních procesů. Abychom se ujistili, že aplikační server plní správně svoji funkci, musí být i on podroben testům. Jednotlivé funkce aplikačního serveru jsou testovány pomocí jednotkových testů, avšak k měření výkonnosti musíme být schopni server nejprve řádně vytížit. K tomu nám slouží benchmarkové aplikace, díky kterým můžeme vyvinout zátěž na požadované komponenty a zkoumat jejich chování v podmínkách, které se co nejvíce blíží těm produkčním.

V úvodní kapitole definuji metriky měření výkonu a popíši klíčové cíle výkonnosti společně s procesem, při kterém nabývají konkrétních hodnot. Ukážu, jakým způsobem lze identifikovat kritické scénáře aplikace, vysvětlím rozdíly mezi jednotlivými typy testů a zmíním se o nástrojích, které nám pomáhají s automatizací.

V následující kapitole vysvětlím pojem benchmarking, popíšu základní typy serverů a ukážu rozdíly, které mezi nimi jsou. Představím Java EE plný profil pro aplikační servery, prostuduji existující možnosti v oblasti benchmarkových aplikací a závěr kapitoly věnuji možnostem optimalizace aplikačního serveru.

Třetí kapitola je věnována popisu architektury aplikace a seznámení se se základními principy Java EE 6 platformy. 

V následujících čtyřech kapitolách projdu všemi vrstvami aplikace od persistentní až po prezentační a představím některé podpůrné technologie. V každé kapitole popíšu specifikace, které byly v aplikaci použity společně s ukázkami kódu některých řešení.

Závěrečná kapitola je věnována několika testům aplikačních serverů. Nejprve představím nástroje, které budou k testům použity, metriky, o které se budu zajímat, a poté jednotlivé testy.

Práce obsahuje dvě přílohy, z nichž první popisuje způsob, jakým je možné aplikaci nasadit na aplikační server, druhá nabízí seznam funkcí dostupných z webového rozhraní aplikace.
...
Plná implementace HTTP protokolu poskytuje také funkcionalitu k přijímání obsahu od klientů. Této schopnosti je využito například při vyplňování webových formulářů nebo při nahrávání dat na server.
...
\subsection{Benchmarking komerčních aplikačních serverů}
...
\subsection{Benchmarking aplikačních serverů s otevřeným zdrojovým kódem}
...
\\\indent Z pohledu JPA se vůbec nic nemění, je možné definovat všechny tři strategie úplně stejně, jako by se jednalo o normální entitu, jediný rozdíl je v tom, že abstraktní entita nemůže být instancializována (stejně tak jako žádná jiná abstraktní třída v jazyce Java).
...
\begin{itemize}
  ...
  \item {\bf messaging} - umožňuje MDB poslouchat na JMS~\footnote{Java Messiging Service - rozhraní umožňující aplikacím asynchronně vytvářet, číst, posílat a přijímat zprávy} destinacích a konzumovat zprávy
...
\chapter{Testování serverů}
V této kapitole předvedeme, jak naší aplikaci použít v testech. K testování jsme použijeme následující nástroje:
\begin{itemize}
  \item {\bf MySQL 5.5.28} - databázový server
  \item {\bf JBoss AS 7.1.1} - aplikační server vyvíjený společností Red Hat
  \item {\bf GlassFish 3.1.2.2} - aplikační server vyvíjený společností Sun
  \item {\bf Apache JMeter 2.8} - nástroj na generování zátěže
\end{itemize}

\section{Srovnání serverů}

Před zahájením testu a okamžitě po jeho skončení změříme paměťové otisky serveru (množství paměti, které server používá ke běhu). Před každým měřením otisků zavoláme několikrát metodu forceGC() Garbage collectoru, abychom zajistili, že v paměti zůstanou pouze nezbytné objekty. 

Java objetky jsou v paměti uloženy na haldě, každá halda je rozdělena do tří částí. Každá část je z pohledu garbage collectoru jinou generací~\cite{performanceTuning}. Při měření otisků nás budou zajímat:
\begin{itemize}
  \item {\bf Eden space} - část haldy, ze které je brána paměť pro tvorbu většiny objektů (na haldě)
  \item {\bf Old gen} - část obsahující objekty, které již existují delší dobu (na haldě)
  \item {\bf Perm gen} - obsahuje definice tříd a různá metadata (mimo haldu)
  \item {\bf Heap memory} - celková velikost haldy
\end{itemize}

Pro každý pokus bude použita nová instance serveru. Každý test bude trvat 10 minut, před spuštění testu vygenerujeme 40 zákazníků a 200 produktů. K aplikaci bude během testu přistupovat soustavně 15 klientů, kteří se budou dotazovat na servlet generující objednávky. Nesmíme zapomenout, že jeden dotaz na servlet není roven jednomu běžnému požadavku, ale vykonání celého scénáře objednávky, který se za normálních okolností skládá z desítek různých požadavků. Klienty budeme generovat pomocí strategie postupného navyšování během prvních deseti vteřin testu. Časová služba na čištění objednávek a faktur bude nastavena na 20 sekund. Zajímat nás budou:
\begin{itemize}
  \item paměťové stopy před spuštěním a po dokončení testu
  \item celkový počet obsloužených požadavků
  \item propustnost
  \item průměrný čas potřebný na obsloužení požadavku
\end{itemize}

\vspace{5 mm}\noindent{\bf Paměťové stopy}
\\\indent Na obráku (viz obrázek~\ref{img:stopy}) vidíme porovnání paměťových stop serverů.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{stopy}
\caption{Paměťové stopy serverů}
\label{img:stopy}
\end{figure}

\vspace{5 mm}\noindent{\bf Výsledky měření}
\\\indent V našem testu si nejlépe vedl (viz obrázek \ref{img:samples}) server GlassFish, který dokázal obsloužit nejvíce požadavků.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{samples}
\caption{Porovnání serverů}
\label{img:samples}
\end{figure}

\section{Nalezení bodu saturace}
V tomto testu si ukážeme, jak se server chová po dosažení bodu saturace. K tomu použijeme Apache JMeter a server GlassFish. 

Tentokrát si před testem vygenerujeme 100 zákazníků a 400 produktů. Na server budeme postupně pouštět 600 klientů pomocí strategie postupného navyšování. Klienti se budou generovat prvních deset sekund testu. Celá dávka se zopakuje pětkrát. Celkem tedy budeme po serveru chtít obsloužit 3000 požadavků. Celý test bude trvat něco málo přes tři minuty.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{saturation}
\caption{Rostoucí křivka odezvy serveru}
\label{img:saturation}
\end{figure}

Na obrázku~\ref{img:saturation} můžeme vidět, že si propustnost drží stejnou hladinu, kdežto čas odezvy s přibývajícím počtem klientů stále roste. Podívejme se na ostatní naměřené hodnoty:
\begin{itemize}
  \item počet vzorků - očekávané 3000
  \item propustnost - 16,4 požadavků za sekundu
  \item průměrná doba odezvy {\bf 31844} milisekund
\end{itemize}
Hodnota 16,4 požadavků za sekundu koresponduje s výsledky předchozích testů, ale průměrná doba odezvy je daleko za požadovanými hodnotami.

\chapter{Závěr}

Cílem práce bylo seznámit se s problematikou výkonnostního testování a s tím souvisejícími problémy, následně navrhnout benchmarkovou aplikaci, která bude použita pro měření a porovnávání výkonnosti aplikačních serverů. 

První kapitolu jsem věnoval podrobnějšímu seznámení se se základními pojmy potřebnými k orientování se v problematice. Ukázal jsem, jakých způsobů lze využít při identifikaci klíčových scénářů. Popsal jsem jednotlivé typy testů společně s jejich významem, zaměřil se na možné způsoby budování zátěže a zmínil se o automatizovaných nástrojích, které jsou při testování nezbytné. 

V další kapitole jsem vysvětlil rozdíly mezi jednotlivými typy serverů a představil jsem Java EE plný profil. Objasnil jsme pojem benchmarková aplikace, popsal problémy, které s porovnáváním aplikačních serverů souvisí, a blíže představil aplikaci Apache DayTrader.

Následující kapitoly jsem věnoval převážně architektuře aplikace a implementačním detailům. Poté, co jsem popsal třívrstvý model a detailněji prošel novinky Java EE platformy, jsem postupně prošel i všechny vrstvy aplikace a zmínil něco o nejzajímavějších specifikacích Java EE, které byly v práci použity.

V poslední kapitole jsem ukázal názorné použití aplikace při srovnávání výkonnosti aplikačních serverů GlassFish a JBoss AS. Nejprve jsem představil nástroje, které jsem k testování použil, a následně prezentoval naměřené hodnoty.

Během studování problematiky porovnávání aplikačních serverů jsem se potýkal s nedostatkem dat, pomocí kterých by bylo možné jednotlivé servery srovnávat. Zatímco IBM a Oracle se hrdě hlásí k výsledkům dosažených v testech organizace SPEC, ostatní spíše jen tiše přihlíží nebo své výsledky nezveřejňují. K dispozici jsou pouze srovnání např. startovacích časů, velikosti distribuce, paměťové náročnosti a jiných. Asi nejvíce používanou aplikací je Apache DayTrader, který je vydáván jako jedna z ukázkových aplikací k serveru Apache Geronimo. S novou verzí Apache Geronimo kompatibilní s Java EE 6 byla vydána i nová verze DayTraderu, tato sice obsahuje komponenty jako entity beany a JSP, ale doufejme, že s příchodem Java EE 7 bude DayTrader opět přepracován.

Jedním z příjemných zjištění bylo nalezení nespočtu podpůrných nástrojů s otevřeným zdrojovým kódem. Právě automatizace je klíčová k dosažení konzistentních a věrohodných výsledků. Já sám jsem během testování použil nástroje na monitorování a na generování zátěže, ale startování serveru a nasazování aplikace jsem dělal manuálně. Tento testovací proces by mohl být předmětem budoucího vylepšení.

Při testování aplikace jsem se potýkal s problémy s transakcemi. Z důvodu délky transakčních dávek a obousměrných vazeb mezi entitami docházelo k uváznutí a zrušení transakcí. Nakonec se mi podařilo tento problém obejít zakomentováním atributu stored u Produktu. Správným řešením by mohla být specifická úprava nastavení poskytovatele JPA nebo fondu JDBC. Dalším z možných řešení by mohl být přechod z transakcí řízených kontejnerem na transakce řízené pomoci bean.

Narazil jsem také na menší problémy při migraci aplikace na GlassFish, kdy metody hashCode() tříd Invoice a Order na sebe vzájemně odkazovaly, čímž docházelo při volání těchto metod k zacyklení. Je zajímavé, že za celou dobu vyvíjení aplikace na serveru JBoss AS, nedošlo k zacyklení ani jednou, ovšem ihned po nasazení aplikace na GlassFish došlo k přetečení zásobníku. Stejně tak jsem řešil různá chování JPA poskytovatelů Hibernate (JBoss) a EclipseLink (GlassFish). GlassFish ovšem obsahuje v administrační konzoli nástroj na doinstalování externích modulů, proto bylo k testování použito Hibernate stejně jako na JBoss. Toto jen ukazuje, jak se mohou imlementace specifikací různých poskytovatelů lišit.

Jedna z možných úprav do budoucna je poskytnutí variabilnější formy zátěže například vytvořením více servletů, které budou simulovat prosté procházení zákazníka stránkami a nebudou volat žádnou náročnější aplikační logiku. V současném stavu je přechod v zátěži mezi každým novým virtuálním uživatelem příliš strmý a nejde budovat variabilnější formu zátěže. K budování zátěže by mohl být použit také skriptovací modul některého z nástrojů, který by přistupoval manuálně k jednotlivým stránkám webového rozhraní.
\appendix
...
Odlehčená verze aplikace~\footnote{aplikace byla zabalena jako webový archiv, není použito JMS a aplikace používá odlehčenou H2 databázi} byla nasazena na platformě OpenShift společnosti Red Hat a je možné s ní pracovat na adrese:
\begin{lstlisting}
