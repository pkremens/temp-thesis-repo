...
V této kapitole si ukážeme, jak pomocí naší aplikace provést několik testů. K testování sme použili následující nástroje:
\begin{itemize}
...
Před zahájením testu a okamžitě po jeho skončení změříme paměťové otisky serveru (množství paměti, které server používá ke běhu). Před každým měřením otisků vynutíme sběr odpadu pomocí JMX~\footnote{Java Management Extensions - technologie podporující management a monitoring aplikací} operace forceGC(), která bude volána na odpovídající managed beaně. Při měření otisků nás zajímají:
\begin{itemize}
  \item {\bf Eden space} - fond ze kterého je brána paměť pro tvorbu většiny objektů (na haldě)
  \item {\bf Old gen} - fond obsahuje objekty, které již existují delší dobu (na haldě)
  \item {\bf Perm gen} - obsahuje definice tříd a různá metadata (mimo haldu)
  \item {\bf Heap memory} - celková velikost haldy
\end{itemize}

Pro každý pokus bude použita nová instance serveru. Každý test bude trvat 10 minut, před spuštění testu vygenerujeme 40 zákazníků a 200 produktů. K aplikaci budeme během testu přistupovat soustavně 15 klientů, kteří se budou dotazovat na servlet generující objednávky. Nesmíme zapomenout, že jeden dotaz na servlet není roven jedenomu běžnému požadavku, ale vykonání celého scénáře objednávky, který se za normálních okolností skládá z desítek různých požadavků. Klienty budeme generovat pomocí strategie postupného navyšování prvních 10 sekund testu. Časová služba na čištění objednávek a faktur bude nastavena na 20 sekund. Zajímat nás budou:
\begin{itemize}
...
V tomto testu si ukážeme, jak se servech chová po dosažení bodu saturace. K tomu použijeme nástroje Apache JMeter a server GlassFish. 

Tentokrát si před testem vygenerujeme 100 zákazníků a 400 produktů. Na server budeme postupně pouštět 600 klientů pomocí strategie postupného navyšování. Klienti se postupně budou generovat prvních deset sekund testu. Celá dávka se zopakuje pětkrát. Celkem tedy budeme po serveru chtít obsloužit 3000 požadavků. Celý test bude trvat něco málo přes tři minuty.
...
Na obrázku~\ref{img:saturation} můžeme vidět, jak se propustnost serveru usadí na relativně konstantní hodnotě, kdežto čas odezvy stále roste. Podívejme se na ostatní naměřené hodnoty:
\begin{itemize}
...
Následující kapitoly jsem věnoval převážně architektuře aplikace a implementačním detailům. Poté, co jsme popsal třívrstvý model a detailněji prošel novinky Java EE platformy, jsem postupně prošel i všechny vrstvy aplikace a zmínil něco o nejzajímavějších specifikacích Java EE, které byly v práci použity.
...
Jedna z možných úprav do budoucna je poskytnutí variabilnější formy zátěže například vytvořením více servletů, které budou simulovat prosté procházení zákazníka stránkami a nebudou volat žádnou náročnější aplikační logiku. V současném stavu je přechod v zátěži mezi každým novým virtuálním uživatelem příliš strmý a nejde budovat variabilnější formu zátěže. 
\appendix
